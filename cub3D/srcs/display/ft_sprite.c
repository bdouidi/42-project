...
/*   Updated: 2020/07/21 21:05:56 by othabchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3D.h"

void	stock_sprite(t_data *d, int i, t_sprite tmp, int *check, int b)
{
	d->spt[i].x[b] = tmp.x[b];
	d->spt[i].y[b] = tmp.y[b];
...
void	catch_sprite(t_data *d, int b)
{
	...
	while (check == 0 && i < d->texture.count_spt)
	{
		if (b == 1)
		{
			while (((int)d->spt[i].x[0] != (int)tmp.x[b] ||
				(int)d->spt[i].y[0] != (int)tmp.y[b]) && 
				i < d->texture.count_spt)
				i++;
		}
		((int)d->spt[i].x[b] == 0 || (int)d->spt[i].y[b] == 0) ?
		stock_sprite(d, i, tmp, &check, b) : 0;
		if ((int)d->spt[i].x[b] == (int)tmp.x[b] &&
			(int)d->spt[i].y[b] == (int)tmp.y[b])
		{
			if (tmp.dist[b] < d->spt[i].dist[b])
				stock_sprite(d, i, tmp, &check, b);
			else
				check = 1;
		}
		else
